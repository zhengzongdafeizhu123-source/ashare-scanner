...
result_file = OUTPUT_DIR / f"p5_scan_from_local_100_diagnose_results_{today_str}.csv"
summary_file = OUTPUT_DIR / f"p5_scan_from_local_100_diagnose_summary_{today_str}.csv"
error_file = OUTPUT_DIR / f"p5_scan_from_local_100_diagnose_errors_{today_str}.csv"
log_file = LOGS_DIR / f"p5_scan_from_local_100_diagnose_{today_str}.log"
...
for idx, file_path in enumerate(files, start=1):
    ...
    try:
        ...
        failed_reasons = []
        if not rule_90d_low_vol:
            failed_reasons.append("90日波动率>20%")
        if not rule_bullish:
            failed_reasons.append("不是阳线")
        if not rule_big_volume:
            failed_reasons.append("未达到3倍放量")
        if not rule_turnover:
            failed_reasons.append("换手率<=10%")

        score = (
            int(rule_90d_low_vol)
            + int(rule_bullish)
            + int(rule_big_volume)
            + int(rule_turnover)
        )
        ...
        results.append({
            ...
            "90日低波动通过": "是" if rule_90d_low_vol else "否",
            "阳线": "是" if rule_bullish else "否",
            "量比前一日": round(volume_ratio, 2),
            "3倍放量通过": "是" if rule_big_volume else "否",
            "换手率": round(latest_turnover, 2),
            "换手率>10%通过": "是" if rule_turnover else "否",
            "命中规则数": score,
...
if result_df.empty:
    ...
    summary_df = pd.DataFrame([{
        "扫描总数": len(result_df),
        "通过90日低波动数量": int((result_df["90日低波动通过"] == "是").sum()),
        "通过阳线数量": int((result_df["阳线"] == "是").sum()),
        "通过3倍放量数量": int((result_df["3倍放量通过"] == "是").sum()),
        "通过换手率>10%数量": int((result_df["换手率>10%通过"] == "是").sum()),
        "全部通过数量": int((result_df["是否入选"] == "是").sum()),
        "失败数量": len(errors)
    }])
